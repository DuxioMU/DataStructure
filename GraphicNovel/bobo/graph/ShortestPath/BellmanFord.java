...
public class BellmanFord <Weight extends Number & Comparable>{
	...
	private int s;			 // 起始点
	private Number[] distTo;//distTo[i]存储从起始点s到i点的最短路径长度
	Edge<Weight>[] from;	//from[i] 记录最短路径中,到达i点的边是那一条
							//可以用来回复整个最短路径
	boolean hasNegativeCycle; //标记图中是否有负权环
	
	//构造函数,使用BelemanFord算法求最短路径
	public BellmanFord(WeightedGraph graph,int s) {
		...
		//Bellman-Ford的过程
		//进行v-1次循环,每次循环求出从起点到其余所有点,最多使用pass步可到达的最短距离
		for(int pass = 1;pass < G.V() ;pass ++) {
			
			//每次循环中对所有的边进行一遍松弛操作
			//遍历所有边的方式是先遍历所有的顶点,然后遍历和所有顶点相邻的所有的边
			for(int i = 0; i < G.V() ; i++) {
				//使用我们实现的临边迭代器遍历和所有的顶相邻的所有边
				for(Object item : G.adj(i)) {
					Edge<Weight> e = (Edge<Weight>)item;
					//对于每一个边首先判断e.v() 可达
					//之后看如果e.w() 以前没有到达过,显然我们可以更新distTo[e.w()]
					//或者e.w()以前虽然到达过,但是通过这个e我们可以获得一个更短的距离,即刻进行一次松弛操作,
					//我们也可以更新distTo[e.w()]
